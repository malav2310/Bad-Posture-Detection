...
import { Camera, Activity, Settings, Eye, BarChart3 } from 'lucide-react';

export default function App() {
  const [isMonitoring, setIsMonitoring] = useState(false);

  useEffect(() => {
    chrome.storage.local.get(['monitoring-state'], (result) => {
  ...
  const toggleMonitoring = () => {
    if (!isMonitoring)
      chrome.runtime.sendMessage({ type: "START_MONITORING" }, () => setIsMonitoring(true));
    else
      chrome.runtime.sendMessage({ type: "STOP_MONITORING" }, () => setIsMonitoring(false));
  };
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex justify-center">
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-lg font-semibold shadow-md shadow-blue-500/40 transition-all"
          >
